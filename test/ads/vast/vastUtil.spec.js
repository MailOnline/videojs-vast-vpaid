...
describe('vastUtil', () => {
  ...
  describe('_parseURLMacro', () => {
    ...
    it('must parse the passed macro using the passed variables', () => {
  ...
  describe('parseURLMacro', () => {
    ...
    it('must parse the passed macro and return the parsed url', () => {
      assert.equal(
    ...
    it('must auto generate CACHEBUSTING variable if not passed  ', () => {
  ...
  describe('parseURLMacros', () => {
    ...
    it('must parse an array of macros and return an array with the parsed urls', () => {
      const macros = [
        'http://foo.bar/[CODE]',
    ...
    it('must auto generate CACHEBUSTING variable if not passed  ', () => {
      const macros = ['http://foo.bar/[CACHEBUSTING]'];
  ...
  describe('track', () => {
    ...
    it('must return an array with the created track images', () => {
      const macros = [
        'http://foo.bar/[CODE]',
        'http://foo.bar/[CODE]/[END]'
      ];
      const trackImgs = track(macros, {CODE: 'BLA', END: 123});
  ...
  describe('parseDuration', () => {
    ...
    it('must return null if the duration is not an string with the format HH:MM:SS[.mmm]', () => {
    ...
    it('must return the duration in milliseconds', () => {
  ...
  describe('parseImpressions', () => {
    let parseImpressions;

    beforeEach(() => {
      parseImpressions = vastUtil.parseImpressions;
    });

    it('must return an empty array if you pass no impressions', () => {
      testUtils.assertEmptyArray(parseImpressions());
    });

    it('must return an empty array if there is no real impression', () => {
    ...
    it('must return an array with the passed impressions formatted', () => {
      const inlineXML = '<InLine>' +
        '<Impression id="DART">' +
        ...
        '</Impression>' +
        '</InLine>';
      const impressionJTree = xml.toJXONTree(inlineXML).impression;
    ...
    it('must add all the passed impressions to the returned array', () => {
      const inlineXML = '<InLine>' +
        '<Impression id="DART">' +
        '<![CDATA[http://ad.doubleclick.net/imp;v7;x;223626102;0-0;0;47414672;0/0;30477563/30495440/1;;~aopt=0/0/ff/0;~cs=j%3fhttp://s0.2mdn.net/dot.gif]]>' +
        '</Impression>' +
  ...
  describe('formatProgress', () => {
    it('must return the formatted progress', () => {
  ...
  describe('parseOffset', () => {
    ...
    it('must return the passed offset string in ms', () => {
      assert.equal(parseOffset('00:00:05.000'), 5000);
    });

    it('must be possible pass the offset as a percentage', () => {
      assert.equal(parseOffset('10%', 1000), 100);
      assert.equal(parseOffset('10.5%', 1000), 105);
    });

    it('with a percentage offset and no duration must return null', () => {
      assert.isNull(parseOffset('10.5%'));
    });

    it('must return null if you don\'t pass an offset', () => {
  ...
  describe('isVPAID', () => {
    it('must return true if the passed mediaFile apiFramework attr is VPAID and false otherwiser', () => {
      assert.isFunction(vastUtil.isVPAID);
      [undefined, false, '', {}, []].forEach((wrongMediaFile) => {
  ...
  describe('findSupportedVPAIDTech', () => {
    const FLASH_APP_MIME = 'application/x-shockwave-flash';
    const HTML5_APP_MIME = 'application/javascript';
    ...
    it('must return Flash tech if it supports the passed mime type', () => {
    ...
    it('must return HTML tech if it supports the passed mime type', () => {
    ...
    it('must return null if no supported tech is found', () => {
      VPAIDFlashTech.supports.returns(false);
      VPAIDHTML5Tech.supports.returns(false);
  ...
  describe('isFlashSupported', () => {
    beforeEach(() => {
      sinon.stub(VPAIDFLASHClient, 'isSupported');
    });

    it('must delegate in VPAIDFLASHClient.isSupported', () => {
